...
Farhan isn't technical and doesn't pretend to be. What he's genuinely good at is dependencies — noticing that Ji-woo's exception queue screen has nothing to display until Tomas's confidence gate exists, three weeks before that becomes a problem.
...
Amara spent six years on the operations floor at a custodian bank before moving into product. That matters more than it sounds: she has personally chased a reconciliation break at 7pm on a settlement date, so when she says a wrong number is worse than a missing number, she is not speaking theoretically.
...
Her contribution to the design is the one nobody expected: she's the reason the exception queue exists at all. The first design just rejected low-confidence documents. Amara asked what Priya was supposed to do with a rejection.

> **Her prompts:** [P06 Write a full PRD](AI-Prompts-Library/phase-1-discovery/P06-write-a-full-prd.md) · [P07 Slice the PRD into stories](AI-Prompts-Library/phase-1-discovery/P07-slice-the-prd-into-stories.md) · [P08 Write acceptance criteria](AI-Prompts-Library/phase-1-discovery/P08-write-acceptance-criteria.md) (with Ananya) · [P14 UI/UX design brief](AI-Prompts-Library/phase-2-design/P14-ui-ux-design-brief.md) (with Ji-woo)
...
Sofia's whole job is telling the difference between a decision you can change next sprint and one you'll be living with in four years. She writes those down — every one of them — as an ADR, a short numbered record of the decision and why.
...
That question is the origin of the confidence gate, the bronze layer, and the argument in [ADR-0003](Case-Study/Python-ETL/artifacts/adr/) that one bad field should reject a whole document — which several people, including Tomas, think is too strict right up until the moment it isn't.
...
If you read the first three books, Rahul is the one who kept saying *"that's not a workflow, that's an agent"* — and he's still doing the same thing here, one level up. His new version is **"that's not a prompting problem, that's a handoff problem,"** which he says roughly once a week for the first month.
...
> **His prompts:** [P01–P05](AI-Prompts-Library/README.md#phase-0--foundation) (all of Sprint 0) · [P15 Implementation plan](AI-Prompts-Library/phase-3-planning/P15-implementation-plan.md) · [P17 Definition of done](AI-Prompts-Library/phase-3-planning/P17-definition-of-done.md) (with Ananya) · [P23 Review someone else's code](AI-Prompts-Library/phase-5-verify/P23-review-someone-elses-code.md) · [P34 Clean up dead code](AI-Prompts-Library/phase-8-improve/P34-clean-up-dead-code.md)
...
Tomas is fast. Genuinely, unusually fast — with an AI he built in three days what the estimate said was two weeks, and it worked.
...
> **His prompts:** [P02 Connect the database](AI-Prompts-Library/phase-0-foundation/P02-connect-the-database.md) · [P13 Design the data contract](AI-Prompts-Library/phase-2-design/P13-design-the-data-contract.md) (with Sofia) · [P18 Implement a story](AI-Prompts-Library/phase-4-build/P18-implement-a-story.md) · [P20 Write tests alongside the code](AI-Prompts-Library/phase-4-build/P20-write-tests-alongside-the-code.md) · [P25 Data quality validation](AI-Prompts-Library/phase-5-verify/P25-data-quality-validation.md) · [P26–P28](AI-Prompts-Library/README.md#phase-6--rework) · [P33 Write the runbook](AI-Prompts-Library/phase-7-release/P33-write-the-runbook.md)
...
Ji-woo works in React and TypeScript, and has one strong opinion that shapes the whole screen: **the user is a person having a working day, not a set of components.**

Priya clears around forty exceptions in a morning. So every extra click in the design isn't one click — it's forty. Every time the PDF viewer loses its scroll position, that's forty times Priya has to find her place again. That framing is why the exception queue ends up keyboard-first, with the PDF and the field list locked in sync, and why the confidence value renders as `82%` rather than `0.8234567`.
...
> **Her prompts:** [P08 Write acceptance criteria](AI-Prompts-Library/phase-1-discovery/P08-write-acceptance-criteria.md) (with Amara) · [P17 Definition of done](AI-Prompts-Library/phase-3-planning/P17-definition-of-done.md) (with Rahul) · [P22 E2E test the application](AI-Prompts-Library/phase-5-verify/P22-e2e-test-the-application.md) · [P24 Find security gaps](AI-Prompts-Library/phase-5-verify/P24-find-security-gaps.md) · [P25 Data quality validation](AI-Prompts-Library/phase-5-verify/P25-data-quality-validation.md)

---

## And the one who isn't at Kestrel

### Priya Raman — Operations Analyst, Northwind

Priya is not on the delivery team. She is the reason there is a delivery team.
...
**Whenever a design decision in this book seems abstract, check it against Priya.** That's what Amara does, and it's why the exception queue exists: the first cut of the design simply rejected documents the machine wasn't sure about, and Amara asked the obvious question nobody had asked.
