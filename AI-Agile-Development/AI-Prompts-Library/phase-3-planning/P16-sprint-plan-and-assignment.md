...
|---|---|
| **Phase** | 3 — Planning |
| **Who runs it** | Project Manager (Atul) |
...
| **Hands off to** | Team Lead + QA (Gautam , Pankaj ), who run [P17](P17-definition-of-done.md) |
...
Monday, nine in the morning. Atulhas a whiteboard, seven people, and eight stories.

Sprint 1 is over. Preetinka Sharma's PRD has been sliced into stories NWD-101 through NWD-108. Hem Singh's spec and ADRs are signed. Dzmitry  has her UI brief. Gautam  spent Friday producing the implementation plan for NWD-103 with [P15](P15-implementation-plan.md), and it's sitting in the repo.

What Atul does not have is a sprint. He has a pile of work and two engineers, and the pile is not evenly shaped: seven of the eight stories are backend and belong to Ravi Mullick. One is the exception queue screen and belongs to Dzmitry.

And there is a problem sitting in plain sight that nobody has said out loud yet. Dzmitry's screen exists to display things the confidence gate rejects. The confidence gate is NWD-103. NWD-103 does not exist. If Dzmitry starts on Monday she has nothing to render, no API to call, and no idea what an exception row looks like on the wire.

Atul has seen two ways this goes wrong. Either Dzmitry waits — three days of a frontend engineer being politely blocked — or she guesses at the shape of the data and builds against her guess, and then spends day eight rewriting when the real shape arrives. He is not enthusiastic about either.
...
|---|---|
| **Backlog** | The full list of everything anyone wants built, ordered by priority. Owned by the Product Owner — Preetinka. Most of it will never be built, and that's fine. |
...
| **Acceptance criteria** | The specific, checkable conditions that make one story done. Per-story. Written by Preetinka with Pankaj, using [P08](../phase-1-discovery/P08-write-acceptance-criteria.md). |
...
> **Watch out.** Sprint 2 is Kestrel's first *build* sprint on this project. Sprints 0 and 1 were foundations and design. There is no build velocity. Anyone who tells you their velocity in sprint one is telling you a number they made up. Atul's plan has to be honest about this instead of inventing a baseline, and §6 shows how.
...
Dzmitry 
  10.0  working days
 − 0.5  ceremonies
 − 1.0  supporting the Sprint 1 design review with Preeti at Northwind
  =====
...
Second, **capacity is not a target to fill**. If Ravi has eight days, you do not plan eight days of work. You plan six or seven, because something will go wrong — a 429 from an Azure service at exactly the wrong moment, a labelled document set that turns out to be mislabelled, a production incident on another account. Atul's instinct here is well-earned: he plans to about 75% and is pleasantly surprised roughly never.
...
> **A Broker Alpha position statement can land in the raw zone, be classified, be gated on confidence, and reach Azure SQL — and anything the gate rejects is visible to Preeti in a browser.**
...
**Assignment** is deciding who does what. In a big team this is contentious; with two engineers and a clean split between backend Python and frontend React, it takes about ninety seconds. Ravi takes NWD-101 to NWD-107. Dzmitry takes NWD-108.

The interesting bit is the **dependency**.

A dependency is when one piece of work can't finish until another exists. NWD-108, Dzmitry's exception queue screen, exists to show a human the documents the confidence gate rejected and why. If NWD-103 hasn't produced any rejections, and there's no table holding them, and no API returning them, Dzmitry has a screen with nothing to put on it.

There are four ways to handle a dependency, and only one of them is any good.

**1. Wait.** Dzmitry does nothing until NWD-103 lands. Costs three or four days of a frontend engineer. Obviously bad, and yet extremely common.

**2. Reorder.** Do NWD-103 first, then start NWD-108. Doesn't help — the backend still takes the same number of days, so Dzmitry still waits, just at the front instead of the back.

**3. Guess.** Dzmitry invents a plausible shape for an exception row and builds against it. Fast right up until the real shape arrives, at which point she rewrites. This is the one teams do accidentally, and it's the expensive one, because the rewrite happens in the last three days of the sprint.

**4. Agree the contract first, then work in parallel against a fixture.** Ravi and Dzmitry spend forty minutes on Monday agreeing exactly what an exception row looks like — field names, types, what `confidence` is, whether `line_item_index` can be null. That agreement is written down. Dzmitry then builds against a static JSON file containing six fake exception rows in that exact shape. Ravi builds the real thing. On day five they swap the fixture for the real endpoint.
...
| "Find every dependency and say how it is handled" | The Dzmitry problem: found on day seven instead of day one |
...
**Judgement** — is this sprint realistic, is this the right goal, is Ravi actually going to manage seven stories. Not reliable. This is Atul's job and the model can only prompt him to do it. The "name the riskiest assumption" instruction exists to get the model to hand the judgement back rather than pretend to make it.
...
|---|---|---|---|
| `[PRD PATH]` | The product requirements from [P06](../phase-1-discovery/P06-write-a-full-prd.md) | `artifacts/prd-counterparty-ingestion.md` | The goal comes out as a technical statement instead of a business one — "the gate works" rather than "Preeti can see rejections" |
...
| `[IMPLEMENTATION PLAN PATH]` | The build sequence from [P15](P15-implementation-plan.md) | `artifacts/implementation-plan-NWD-103.md` | You lose the day-level detail — the plan can't tell you the exceptions table lands at Step 5, so it can't tell Dzmitry when to expect it |
...
| `[TEAM, ROLE, AVAILABLE DAYS, AND WHAT IS EATING THE REST]` | Every person, with the subtractions spelled out | `Ravi Mullick — backend — 8.0 of 10 (1d leave Thu wk1, 0.5d ceremonies, 0.5d Sprint 1 carry-over)` | The single highest-value input. Wrong here and every other number in the plan is wrong |
...
Atul runs this at 8:15am on the Monday, forty-five minutes before the planning session, having collected everyone's availability in Slack on Friday.
...
**The team and their real availability:**
- Ravi Mullick — Backend Engineer (Python, Azure) — 8.0 of 10 days.
  1.0 day annual leave Thursday week one; 0.5 day ceremonies; 0.5 day Sprint 1
  carry-over finishing the bronze persistence review with Hem.
- Dzmitry  — Frontend Engineer (React, TypeScript) — 8.5 of 10 days.
  0.5 day ceremonies; 1.0 day supporting the design walkthrough with Preeti Singh
  at Northwind on Wednesday week one.
- Gautam  — Team Lead — not taking stories. Reserve 0.5 day per day for code
  review, pairing and unblocking.
- Pankaj  — QA — writing the E2E harness this sprint, not taking build
  stories. She needs a working exception queue by day 8 to start against.
- Hem Singh — Architect — 1 day total, on call for design questions.
- Preetinka Sharma — Product Owner — available for acceptance, not building.
- Atul— Project Manager — me.
...
Northwind counterparty ingestion · 10 working days · Prepared by Atul
...
> visible to Preeti in a browser.
...
The recommended goal also names Preeti. That is intentional: it is the only
version of the goal that Preetinka can judge without an engineer translating it.
...
Gautam (0.5 day/day review + unblocking), Pankaj (E2E harness), Hem (1 day
design support) and Preetinka (acceptance) are not counted as build capacity. That is
...
set needing correction, and rework arising from Gautam's reviews. It is not spare
...
| NWD-105 | Redact PII before anything is persisted | 5 | Ravi | **Yes** for the goal, **no** for go-live. See risk below |
| NWD-106 | Transform extracted fields into the canonical position schema | 5 | Ravi | **No** — nothing reaches SQL without it |
| NWD-107 | Load positions into Azure SQL and Snowflake idempotently | 8 | Ravi | **Partly** — Azure SQL half is required, Snowflake half is not |
| NWD-108 | Exception queue screen for analyst review | 8 | Dzmitry | **No** — "visible to Preeti" is half the goal |

Ravi carries 34 points; Dzmitry carries 8. That imbalance is real and it is not
solved by moving points around — Dzmitry cannot write the Python and Ravi is not
building React this sprint. It is managed by the drop order in §7: four of
Ravi's 34 points are explicitly droppable, and Dzmitry's spare 4-5 days are
...
- **Why it is critical:** NWD-108 is Dzmitry's entire sprint. Blocked, she has
...
Day 1, before either of them writes anything: Ravi and Dzmitry agree the exception
record shape against `data-contract-counterparty-position.md` §4 and
`implementation-plan-NWD-103.md` Step 5. Forty minutes, both present, Gautam in
...
Dzmitry builds against that fixture until day 5. Ravi's Step 5 (exceptions
...
Dzmitry half a day she does not have.

### D2 — NWD-103 needs NWD-102 (in-person, low risk)

The gate resolves per-counterparty thresholds, so it needs to know the
counterparty. Both stories are Ravi's, in sequence, so this is a sequencing note
rather than a coordination risk. Implementation plan Step 2 depends on it.

### D3 — Pankaj needs a working exception queue by day 8

Not a story dependency, a people one. Pankaj cannot start the E2E harness against
...
Dzmitry has roughly 4 days of slack after NWD-108's core screens. Committed to,
in order: the loading/empty/error states the UI brief calls for, keyboard
accessibility for Preeti's actual working pattern (she does this 200 times a day
and will not use a mouse), and pairing with Pankaj on the E2E selectors so the
...
3. **NWD-105 (redaction).** The goal survives without it. **Caveat: this cannot
   ship to production without it**, and dropping it must be a decision Preetinka and
   Hem make consciously, not a thing that quietly happens on day nine. If it
...
**That NWD-103 lands by day 5.** Everything downstream of it — Dzmitry's real API
integration, Pankaj's E2E start, and the second half of Ravi's sprint — is
scheduled behind it, and Ravi loses day 4 to leave.
...
**What "done" means here.** The plan is done when Atul can walk into the room and defend three things without opening another document: why this is the goal, what comes out first if things go badly, and what has to happen on day one for Dzmitry not to be blocked.
...
| Dependencies say "coordinate with Dzmitry" | Detected but not handled — no day, no fixture, no owner | §8.3 |
...
What changes: the imbalance stops being hidden and becomes two explicit decisions. Point 3 is the one Atul actually cares about — with two engineers and no overlap, one person off sick is a sprint-level event, and it's better said out loud in the plan than discovered on a Tuesday.
...
Atul sends the plan to Northwind. Somebody at Northwind reads "Sprint 2 delivers NWD-101 to NWD-108" and writes it in a steering pack. Three weeks later the sprint delivered six of eight and Kestrel is explaining a shortfall against a number nobody on the team ever committed to.
...
The fix is a single sentence at the top and the drop order at the bottom. "This is a forecast; Sprint 3 will be the first plan with a measured baseline behind it" costs nothing and changes how the whole document is read. Atul writes it every first sprint and has never regretted it.
...
**A dependency in a plan is not handled until it has a date, an owner and an artifact.** "We'll coordinate" is a hope. "Ravi and Dzmitry, day one, forty minutes, output is `exceptions.sample.json` committed to the repo" is a plan. The difference is about ninety seconds of writing.
...
Ravi is on this project full time, except when he isn't. There's the support rota. There's the interview panel on Thursday. There's the twenty minutes a day answering questions about last quarter's project that nobody has counted since March.
...
The fix is in the input, not the prompt: collect real availability on the Friday before, in writing, from each person, including the things they feel slightly awkward mentioning. Atul asks a specific question rather than an open one — "what is definitely taking you away from this sprint?" — because "are you free?" gets you "yes" and then a training day nobody knew about.
...
What actually changes in the room is usually the capacity numbers (someone remembers a commitment) and the dependency mitigation (the engineers know something about the shape of the work the PM doesn't). What rarely changes is the goal and the drop order, because those are Preetinka's call and she is in the room to make it.

Gautam and Pankaj pick it up next, and they run [P17](P17-definition-of-done.md). The connection is direct: the plan committed 12.4 days of work, and the estimates behind that number are only meaningful if everyone agrees what "done" includes. If Ravi thinks done means the tests pass and Pankaj thinks it means she's signed it off, the capacity arithmetic in §2 is fiction. The Definition of Done is what makes the number mean something.

Dzmitry's first action out of this plan happens before any prompt: the forty-minute conversation with Ravi on day one that produces the fixture. Then she goes to [P19](../phase-4-build/P19-build-the-ui-from-the-brief.md) with her UI brief and that fixture in hand. Ravi goes to [P18](../phase-4-build/P18-implement-a-story.md) with NWD-101 and the implementation plan.
...
The interesting thing that happened is that the first draft got D1 backwards. It correctly spotted that NWD-108 depended on NWD-103, and then recommended mitigation (b) — reorder, do NWD-103 first. Atul read that and immediately saw the flaw: reordering doesn't shorten the backend work, it just moves Dzmitry's idle days from the end of the sprint to the start. He ran the §8.3 follow-up and got the fixture approach, which is what the team actually did.

The conversation that came out of it lasted thirty-five minutes rather than the forty budgeted, and produced `exceptions.sample.json` with seven rows rather than six. Dzmitry added the seventh herself: a document where the same field failed twice at different line-item indices, because she wanted to know whether to group by field or by row before she wrote a single component. Ravi hadn't thought about it. The answer changed one line of his Step 6.
