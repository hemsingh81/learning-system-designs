...
|---|---|---|
| Ravi Mullick (Backend) | 9 | One day on Sprint 3 discovery for the Aladdin pull |
| Dzmitry  (Frontend) | 8 | Two days lost to the design-system upgrade |
| Pankaj  (QA) | 7 | Half-time until day 4; the E2E harness needs the pipeline to exist first |
| Gautam  (Team Lead) | 5 | Review, pairing, and unblocking. Not counted as delivery capacity |
...
|---|---|---|---|---|---|
| [NWD-101](stories/NWD-101.md) | Land PDFs immutably in the raw zone | 2 | Ravi | 1 | ✅ d1 |
| [NWD-102](stories/NWD-102.md) | Classify a PDF to its counterparty layout | 5 | Ravi | 2 | ✅ d3 |
| [NWD-103](stories/NWD-103.md) | **Gate every field on its confidence score** | 8 | Ravi | 3 | ✅ d6 |
| [NWD-106](stories/NWD-106.md) | Transform to the canonical position schema | 3 | Ravi | 7 | ✅ d8 |
| [NWD-107](stories/NWD-107.md) | Load to Azure SQL and Snowflake idempotently | 5 | Ravi | 8 | ✅ d10 |
| [NWD-108](stories/NWD-108.md) | Exception queue screen for analyst review | 8 | Dzmitry | 4 | ✅ d10 |
...
**NWD-108 — Dzmitry's exception queue — has nothing to display until NWD-103 exists.** The screen's entire job is to show an analyst *which field failed, what value the machine read, what confidence it had, and what threshold it missed*. All four of those come out of the gate's failure output. Until the gate exists, there is no shape to render.

On the naive plan, Dzmitry starts NWD-108 on day 1, discovers on day 1 that there is nothing to build against, and either waits five days or invents a shape that the gate then contradicts.

Three options considered:

| Option | Cost |
|---|---|
| Dzmitry waits for NWD-103 | 5 idle days, and the sprint goal becomes unreachable |
| Dzmitry invents the shape and reconciles later | Rework, and the reconciliation lands in the last two days of the sprint where there is no slack |
| **Freeze the failure shape as a contract on day 1, before either side is built** | **One hour of Hem's time on day 1** |

We took the third. Hem wrote the failure output shape into [`spec-confidence-gate.md`](spec-confidence-gate.md) §4 on day 1, before Ravi started the gate and before Dzmitry started the screen. Both built against the frozen shape rather than against each other's code.
...
Dzmitry built against that JSON with a hand-written fixture for four days before a real one existed. When they finally connected on day 8, it worked first time.
...
| Dzmitry blocked on NWD-103's output shape | **Mitigated by the day-1 contract freeze.** Did not materialise |
| Pankaj cannot write E2E tests until the pipeline runs end to end | Materialised. E2E started day 8, ran into Sprint 3. Accepted |
| Ravi is the single point of delivery for six of seven stories | Materialised and unaddressed. See below |

---

## 6. Sprint close — what actually happened

**All six committed stories delivered. Sprint goal met on day 10.** A Broker Alpha PDF goes in one end and comes out as rows in Snowflake or as an exception on Preeti's screen.

Two things worth recording honestly.

**Ravi finished NWD-103 in three days against an eight-point estimate.** The gate logic itself took about ninety minutes; the rest was thresholds and tests. This looked like excellent news at the time. It is the same speed that let a serious defect through undetected two weeks later — see [NWD-142](bug-NWD-142.md) and the [retrospective](retrospective-sprint-3.md). The lesson is not that Ravi went too fast. It is that our **verification** did not speed up to match, so the ratio of code-produced to code-genuinely-checked quietly got worse and nobody was measuring it.

**Six of seven stories on one person was a real risk and we ran it anyway.** It worked, and it also meant one person's mental model was the only mental model of the pipeline. Gautam's review caught style and one real defect; it did not catch a missing concept, because you cannot review for a concept nobody in the room has.
...
>
> Produced by: Atul(Project Manager), using [P16](../../../AI-Prompts-Library/phase-3-planning/P16-sprint-plan-and-assignment.md)
