...
Atul opens standup by reading out the state of things, which takes forty seconds:
...
Ravi's NWD-142 branch is a working tree with about four hundred lines of change in six files. It merged on Friday afternoon as one commit called `fix NWD-142`, which Gautam let through because it was Friday afternoon and it was blocking the release branch.

On Monday he asks Ravi to redo it before it goes into `release/1.0.0`.

> **Gautam:** "In four months somebody's going to `git blame` line 780 of `rules.py` and get a commit called `fix NWD-142` with a four-hundred-line diff. What are they going to learn?"
...
Ravi writes the runbook on Tuesday with [P33](../../AI-Prompts-Library/phase-7-release/P33-write-the-runbook.md), and the framing Gautam gives him is the reason it comes out useful:
...
| Alert `exception_queue_depth > 40` | More documents than Preeti can clear in a morning are waiting. Usually one counterparty changed a layout overnight. | Group `etl.extraction_exception` by `source_key` and `reason`. If one reason dominates, it is a layout change, not forty separate problems. Tell Preetinka before you tell Preeti. | Do not raise thresholds to clear the queue. That is switching off the control to make its alarm stop. |
...
| Reason `low_confidence: quantity` spiking on one counterparty | Scan quality dropped — a new scanner, a fax, a different template. | Compare against last week's documents for the same source. If the PDFs genuinely look worse, tell the counterparty. Threshold changes go through Hem, never on call. | Do not lower the threshold. £-denominated quantities are gated at 0.90 for a reason and it is written down in spec §3. |
...
>
> **Never make a threshold, rule or configuration change while on call.** Wake Hem instead. She has agreed to this in writing.

---

## 4. P32 — the readiness pack

Wednesday. Atul and Gautam run [P32](../../AI-Prompts-Library/phase-7-release/P32-release-readiness-check.md) together. The output is `artifacts/release-readiness-v1.0.md`, and the useful thing about it is that it is not a checklist of green ticks — the prompt forces every item to have evidence and an owner, and forces an explicit list of what is knowingly not ready.
...
| NWD-108 exception queue | PASS | NWD-139 closed; Preeti signed off on the demo 17 Jul |
...
| **Straight-through rate** | **ACCEPTED RISK** | **58% against a target of 85%. Multi-page statements are refused rather than recovered until NWD-145 lands. Preeti's workload increases in the interim, agreed by Preetinka 30 Jul** |
...
> "Everything passes. Your tests pass, your data quality checks pass, you've found and fixed five defects, and you've been running against real files for three weeks. Preeti is spending four hours a day on something the system now does. Why are we asking her to do it for another month?"

**A parallel run means running the new automated process and the old manual process side by side, on the same documents, every day, and comparing the two outputs.** Preeti keeps typing PDFs into her spreadsheet. The pipeline keeps loading them into Snowflake. Every morning someone compares.

It is expensive. It is dull. It is the thing everyone wants to skip and it is where the argument lands.

Preetinka says no. Hem says no. They give completely different reasons and both reasons are correct.

### Hem's reason: a control cannot validate itself
...
There is a regulatory version of the same argument, and Hem makes it second because it is the weaker one: this pipeline is a **control** in the accounting sense. It sits between a counterparty's statement and a number that reaches a valuation. Replacing a manual control with an automated one is a change an auditor will ask about, and the evidence they will ask for is a period of parallel operation with documented comparison. Not a test report.

### Preetinka's reason: she has been the analyst

Preetinka's reason takes twenty seconds and lands harder, because she does not argue about controls at all.
...
>
> If we cut this over on the 17th and something we haven't thought of goes wrong in week three, the person sitting in that conversation is Preeti. She'll be defending numbers she didn't produce, from a system she was told to trust, and she'll have no way to show it wasn't her.
>
...
**Notice that neither of them argued about the software.** Hem argued about epistemology — what can count as evidence — and Preetinka argued about who carries the blame. The engineers in the room, who knew the code best, had the least useful things to say, and that is normal and worth noticing.

### What they actually agree

Atul writes it up as the cutover gate, and the specificity is the point — "run in parallel for a while and see how it goes" is a plan that never ends.

> **Cutover gate — v1.0 counterparty ingestion**
>
> **Duration.** Minimum two weeks. Maximum four. Must include one month-end.
>
> **What runs.** Both processes, on every counterparty document, every business day. Preeti keys documents as she does today. The pipeline processes the same documents independently. Neither sees the other's output.
>
> **The comparison.** A daily automated diff of Preeti's spreadsheet against `silver.counterparty_position`, on account number, security identifier, quantity and market value. Produced by 09:30, reviewed by Preeti and one of Ravi or Pankaj, signed by both.
>
> **The gate.** Cutover is approved when **all** of these hold:
>
> 1. **Zero divergence on auto-accepted rows for ten consecutive business days.** Not "low divergence". Zero. A row the pipeline accepted without human touch must match Preeti's keying exactly, within the reconciliation tolerances (0.0001 quantity, 0.005 market value).
> 2. **Every document Preeti keyed is accounted for** — either loaded, or in the exception queue with a reason. No document is unexplained.
...
> 4. **Every divergence investigated to a root cause** and written down, including the ones where Preeti was wrong. Especially those.
>
...
>
> **Who signs.** Preetinka for the business, Hem for the architecture, Northwind's operations director for the client.

Clause 1 is the one people push back on, and Hem holds the line on it for a specific reason worth spelling out:
...
Clause 4's last sentence — *especially the ones where Preeti was wrong* — is Preetinka's. She knows the parallel run will find transcription errors in the manual process, because the manual process is a person reading a scanned fax at 8:40 in the morning. Those divergences are the ones that prove the system is worth building, and they are also the ones nobody would write down unless you told them to in advance.
...
- **Six** were Preeti. Transposed digits, mostly, on scanned Broker Alpha statements. One was a whole line skipped on a two-page statement, which is the manual-process version of NWD-142 and got a wry note in the retro.
- **Three** were rows the pipeline had sent to the exception queue, where Preeti's correction differed from what the model extracted. Not gate failures. The gate working.
...
- **One** was a currency-rounding difference in Preeti's spreadsheet formula that had been there since 2023 and had never been noticed.
...
And one thing that is not an artifact: Atul has now watched a sprint estimated at one day of bug fixing consume six days of rework, and he has a retrospective to run about it.
