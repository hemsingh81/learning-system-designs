...
> **One line:** the finding is not that Tomas made a mistake — it is that nothing anywhere in the team's process ever asked whether all the data had arrived, and the best control in the system was the reason nobody noticed.

---

## 1. Thursday, 13 August, 10:00

Seven people in a room, or four in a room and three on a call, which is what these things actually look like. Farhan Qureshi has a laptop and a document with one heading in it.
...
Farhan opens with the same two sentences he opens every retro with, and he means them:
...
Farhan puts the arithmetic up first, because [P35](../../AI-Prompts-Library/phase-8-improve/P35-run-the-retrospective.md) runs in two passes and the first pass is the timeline, not the opinions. **A room that discusses feelings before facts remembers the sprint it thought it had.**
...
| Test — Ananya's end-to-end suite against 40 real documents | 3 | **2** |
| Bug fixing | **1** | **6** |
| Slack | 1 | 0 |

Six days of rework against an estimate of one.

Not six days of Tomas. Six days of engineering time across the team: Tomas for a day and three quarters on NWD-142 alone, Sofia for most of an afternoon on the specification change, Amara on the approval, Ananya re-testing, Rahul reviewing twice, and then four more defects that were smaller and still not free.

Farhan's line, and he says it before anybody else can:
...
| Date | Event |
|---|---|
| 17 Jul | NWD-103 marked done. Reviewed, merged, accepted by Amara |
| 22 Jul | Ananya begins end-to-end testing against 40 real counterparty documents |
...
| 30 Jul, 08:15 | Tomas reaches for the debugging prompt. Wrong tool. 25 minutes lost |
...
| 30 Jul, 11:15 | Sofia's counter-example kills the obvious fix. It becomes a specification problem |
| 30 Jul, 13:55 | Amara approves the spec amendment, with NWD-145 raised and dated |
...
The prompt asks for this first and the room usually wants to skip it. Farhan does not let them, for a reason that is practical rather than pastoral: **the things that worked are the things you are about to accidentally stop doing.**

1. **Ananya's bug report was good enough to prompt with directly.** [`bug-NWD-142.md`](artifacts/bug-NWD-142.md) carried the exact statement, the page count, the expected and actual row counts, the break IDs, and — crucially — the words `GUESS, not verified` in capitals in front of her theory. Tomas pasted it in whole and got a working reproduction on the first attempt.
2. **The bronze layer turned a production defect into an offline test in thirty seconds.** Sofia argued for storing the raw API response before parsing on cost grounds in Sprint 1. The payoff was debugging, and nobody predicted it.
...
4. **Tomas raised NWD-142 as a specification problem rather than patching around it.** The merge was green at 10:52 and he did not ship it. That is not obvious behaviour and it is the reason the spec is not now a lie.
...
**This is not a PDF problem, a table problem or an Azure problem.** It is a class of failure the system had no defence against anywhere, and two instances of it were already running in the dev environment before Ananya found the third.
...
Sofia designed it. Rahul reviewed against it. Ananya wrote acceptance tests around it. Amara wrote acceptance criteria that assumed it. Tomas built it and can explain every line. All five carried the same mental model, in which the confidence gate is *the thing that stops bad data reaching the warehouse*.

It stops bad **values**. The gap between those two statements is where NWD-142 lived for twelve days.

Sofia's framing in the room, which Farhan writes down verbatim:

> "We never asked what class of error the gate specifically does not see. We asked whether it worked. It does."

And Ananya's, which is shorter and is the one that ends up in the release pack:
...
Sofia's standing question — *what does this look like when it's wrong?* — turns out not to be quite enough on its own, and she is the one who says so. A wrong value is visible. Missing data is not. The question has to be asked in a form that reaches absence.

---

## 6. Finding 3 — this retro is two weeks late, and that is a finding

Sprint 3's retro was scheduled twice and dropped twice, because the rework consumed the time. The same pressure slipped two of Rahul's code reviews past the one-working-day commitment.
...
The room considers a rule that the retro cannot be moved, and rejects it. A rule nobody can keep under pressure is worse than an honest acknowledgement, because breaking it teaches everyone that the rules are decorative. What Farhan does instead is move the retro to the last day of the sprint rather than the first day of the next, so that cutting it is a visible decision rather than a scheduling drift.
...
**Action 3 is the only one that changes a person's job rather than the system.** Farhan proposed it about himself, unprompted, which is the healthiest thing in the document. Rework was invisible in the plan, so it happened in the gaps, so the sprint was late for reasons nobody could point at.
...
| "Sofia should review all extraction code" | Creates a bottleneck and puts the fix in a person rather than in a process | Action 2, which puts it in the template |
...
Wednesday afternoon, before the retro. Rahul runs [P34](../../AI-Prompts-Library/phase-8-improve/P34-clean-up-dead-code.md), which is a different kind of tidying-up from the one people expect.
...
Rahul writes one sentence into the deletion commit that is worth more than the deletion:
...
This one was added with the NWD-142 fix, as a kill switch in case the new completeness rules proved noisy and flooded Priya's queue. They did not. It defaults to `True`, no environment overrides it, and the Function app's settings do not contain it.
...
Removing it is not tidying. It is a design statement: it makes the completeness check unconditional, which is a thing the specification should say, so the spec is updated in the same commit. Rahul flags that explicitly rather than deleting quietly, and Sofia signs it off.
...
Friday. Sofia and Rahul run [P36](../../AI-Prompts-Library/phase-8-improve/P36-tech-debt-triage.md), which is a different exercise again and the one most often done badly.
...
Tomas shipped it that way in Sprint 2 to get two stories landing in the same week, and recorded it nowhere. **The date pressure that justified the trade is gone; the trade has expired.**
...
Farhan's last question in the retro. One or two lines each, no preparation, going round the room.

**Farhan Qureshi, project manager.**
...
**Amara Osei, product owner.**
> "Ask 'rejected to where?' about everything the system refuses. Every refusal ends up on a person's desk, and if you haven't decided which person, you've decided it's Priya."

**Sofia Marchetti, architect.**
...
The system works. It ships on 4 September after a nineteen-day parallel run. Manual keying is gone, breaks surface on T+1 instead of T+2, and Priya Raman spends her mornings on the documents that are actually hard.
...
And the one thing that caught NWD-142 was not an artifact at all. It was Ananya looking at a break report and thinking *that is not the shape settlement failures make*, and then opening the PDF and counting the rows with her finger.
