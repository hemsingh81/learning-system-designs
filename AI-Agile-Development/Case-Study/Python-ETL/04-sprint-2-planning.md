...
Farhan Qureshi is in the office ninety minutes before anyone else, which is not unusual, and he is doing arithmetic, which is.
...
Tomas Vargas: ten, minus a day's leave on Thursday, minus finishing the bronze persistence review with Sofia. Ji-woo Park: ten, minus a day at Northwind with Priya Raman.
...
1. **Rahul Nair** turns Sofia's spec into a numbered build sequence for NWD-103 — [P15](../../AI-Prompts-Library/phase-3-planning/P15-implementation-plan.md). He actually did this on Friday at half past four, for reasons [Chapter 3](03-sprint-1-design.md) explains, and this morning he does the thing he should have done then.
2. **Farhan** plans the sprint around it — [P16](../../AI-Prompts-Library/phase-3-planning/P16-sprint-plan-and-assignment.md).
3. **Rahul and Ananya Iyer** write down what "done" means, once, for every story — [P17](../../AI-Prompts-Library/phase-3-planning/P17-definition-of-done.md).
...
Rahul's framing at standup, which is the sentence Tomas remembers all sprint:
...
So the plan's job is not to describe the design. Sofia's spec does that. The plan's job is to **cut the design into pieces small enough that a human can honestly say they read one.**
...
**If the "Verify" line on a step is weaker than the green command, the step is not verified.** That single check is how Rahul reads a plan in thirty seconds.
...
Rahul tells the prompt to **order by risk**, where risk means *the thing that, if the answer surprises us, throws away work already done*. And he names what he thinks the riskiest thing is, so the model has something to disagree with:
...
| R3 | Is there an existing exceptions table anywhere, or does this story create it? | `sql/schema.sql` has only `positions_staging`. If NWD-108 assumes a table shape, we must agree it before Step 5. | Ask Ji-woo — 5 min |

And the line under it, which is the model doing the useful half of its job:

> I agree R1 is the riskiest. R3 is not technically risky but it is a cross-person dependency and it blocks Ji-woo, so it is answered before Step 5.

**R3 is the sentence that makes Farhan's morning easier three hours later.** A plan for one engineer's story noticed that a different engineer is blocked by it, because it was told to read all the artifacts rather than just the one it was planning.
...
- **Change:** Accept an ordered sequence of line items alongside the header fields. Evaluate every cell of every row. Record the row index on each failure so the exception queue can point Priya at row 34 rather than "somewhere in the table". Any single failure sets `passed = False` for the whole document.
...
|---|---|---|---|
| **4** | Call the gate from the rules engine | `core/rules.py`, `tests/test_rules.py` | A rejected document stops silently. Nothing reaches Priya |
...
> - **Step 3** — the line-item loop is where an off-by-one on the row index will send Priya to the wrong row, and she will not be able to tell that it is wrong.
...
The plan is saved to [`artifacts/implementation-plan-NWD-103.md`](artifacts/implementation-plan-NWD-103.md), in the repository, where Farhan can read it before the planning session. That matters in about forty minutes.

---

## 4. Farhan's sprint plan

### The goal, and the two candidates

Farhan asks for two candidate sprint goals and a recommendation, which is a small trick that gets a much better first one.

> **Recommended:**
> A Broker Alpha position statement lands in the raw zone, is classified, is gated on confidence, and reaches Azure SQL — and anything the gate rejects is visible to Priya in a browser.
>
...
The recommended goal also names Priya, and Farhan says so out loud in the room: it is the only version Amara Osei can judge without an engineer translating it for her.
...
**Capacity is not a target to fill.** Farhan commits to 75% of 16.5, which is **12.4 days of planned work**, and he writes down what the remaining 4.1 days are reserved for rather than leaving it as a vague cushion: Azure service surprises, the labelled-document set needing correction, and rework arising from Rahul's reviews.

Rahul, Ananya, Sofia and Amara are not counted as build capacity at all. Rahul is reserving half a day a day for review, pairing and unblocking. Farhan's note in the plan:
...
Farhan has none, and the plan says so in a sentence he could easily have softened and did not:
...
What he uses instead is Rahul's implementation plan: eight steps, roughly 350 lines of production code plus tests, which reads bottom-up as about two and a half days for NWD-103 including review and rework.
...
| NWD-105 | Redact PII before anything is persisted | 5 | Tomas | **Yes** for the goal, **no** for go-live |
| NWD-106 | Transform extracted fields into the canonical position schema | 5 | Tomas | **No** — nothing reaches SQL without it |
| NWD-107 | Load positions into Azure SQL and Snowflake idempotently | 8 | Tomas | **Partly** — Azure SQL half required, Snowflake half not |
| NWD-108 | Exception queue screen for analyst review | 8 | Ji-woo | **No** — "visible to Priya" is half the goal |

Tomas carries 34 points. Ji-woo carries 8. Farhan writes the uncomfortable line rather than shuffling the numbers to hide it:

> That imbalance is real and it is not solved by moving points around — Ji-woo cannot write the Python and Tomas is not building React this sprint.

---

## 5. The dependency, found three weeks early

This is the section the sprint turns on, and it is section 5 of the plan because Farhan told the prompt to treat dependencies as the most important part of the document.

> **A dependency** is when one piece of work cannot finish until something in another piece of work exists.

Here is this one, in plain terms. **NWD-108 is Ji-woo's exception queue: the screen where Priya fixes a document the pipeline refused.** It shows a list of refused documents and, for each one, which field failed and why. **NWD-103 is the confidence gate: the thing that does the refusing.**

Until NWD-103 exists and has decided what a refusal looks like — the column names, the types, whether `line_item_index` can be null — Ji-woo has a screen with nothing to put on it. Not "not much". Nothing.

Farhan's plan states it as a fact with a date on it:
...
> - **Why it is critical:** NWD-108 is Ji-woo's entire sprint. Blocked, she has nothing else assigned.

### The four options, and why three of them are bad

| Option | Assessment |
|---|---|
| (a) Wait | Ji-woo does nothing until NWD-103 lands. Costs four to five days of a frontend engineer. Obviously bad, extremely common. Rejected. |
...
> **A fixture** is a fixed, fake piece of data you develop against so you do not need the real system running. Ji-woo's is a JSON file with six invented exception rows in it.

**The expensive part of a dependency is not the code. It is the agreement.** Once Tomas and Ji-woo have agreed exactly what an exception row looks like, the two sides are genuinely independent — she builds against a file, he builds against a database, and on day six they swap the file for the real endpoint and nothing needs rewriting. Until they have agreed, no amount of scheduling helps, because whatever she builds is a guess that gets rewritten in the last three days of the sprint.

The plan does not say "coordinate with Ji-woo." It specifies the meeting and the artifact:

> Day 1, before either of them writes anything: Tomas and Ji-woo agree the exception record shape against `data-contract-counterparty-position.md` §4 and `implementation-plan-NWD-103.md` Step 5. Forty minutes, both present, Rahul in the room. Output is a committed `exceptions.sample.json` fixture containing six rows and covering, at minimum:
>
...
**Those four bullets are the whole value of the section.** A fixture that only contains tidy rows produces a screen that only handles tidy rows, and the awkward cases arrive in production. A null `line_item_index` is exactly the sort of thing that gets designed out of a fixture on Monday and turns up in front of Priya in October.

The other three dependencies are shorter and one of them is not about software at all:

- **D2 — NWD-103 needs NWD-102.** The gate resolves per-counterparty thresholds, so it needs to know the counterparty. Both stories are Tomas's, in sequence. A sequencing note, not a coordination risk.
- **D3 — Ananya needs a working exception queue by day 8.** Not a story dependency, a people one. She cannot start the end-to-end test harness against a screen that does not render. Flag at standup on day 6 if it looks tight.
...
This sketch is wrong by Wednesday, and Farhan says so when he puts it up. It is drawn from point estimates that are guesses on a team with no velocity. **The only load-bearing thing in it is the arrow, because the arrow is D1's deadline.**

---

## 6. "What happens if that takes twice as long?"

Farhan asks this in every planning session anybody has ever attended with him, and most of the time it is a mild irritant. People have learned to answer it quickly so the meeting can continue.

Today it lands, and it lands on the arrow.

> **Farhan:** "Step 5 is the exceptions table. Your plan says day five. What happens if that takes twice as long?"
>
> **Tomas:** "It won't. It's a `CREATE TABLE`. Twenty-five lines of DDL."
>
> **Farhan:** "I believe you. What happens if it does?"
>
> **Tomas:** *(pause)* "Then Ji-woo swaps the fixture on day eight instead of day six."
>
> **Ji-woo:** "No, then I swap it on day eight and find out on day eight whether I guessed the shape right. If I guessed wrong I've got two days to rewrite a screen and Ananya's got nothing to test."
>
> **Rahul:** "So the risk isn't Step 5 being late."
>
> **Ji-woo:** "The risk is the *shape* being late. The table can be Thursday. The column names have to be Monday."
...
| What Ji-woo needs | When | Why that date |
|---|---|---|
...
**Splitting one deadline into two is the entire output of the question, and it is worth more than the rest of the session.** The forty-minute conversation moves to nine o'clock on the Monday morning, before either of them opens an editor, and it happens before Tomas has any code that might make him defensive about the shape.

Farhan's question is not a challenge to the estimate. It never is. It is a request for the **failure shape** — *what specifically goes wrong, and to whom* — and the useful answer almost never comes from the person being asked. It came from Ji-woo, who was not being asked at all.
...
Monday afternoon. Rahul and Ananya at one desk with [P17](../../AI-Prompts-Library/phase-3-planning/P17-definition-of-done.md), an hour before they take it to the team at four o'clock.
...
Most Definition of Done templates you can find online were written before four hundred correct-looking lines could arrive in ten seconds. They cover tests, review and deployment, and they are fine as far as they go. Rahul and Ananya add three that those templates do not have.
...
**D7 gets pushback, in the room, from Tomas, and the objection is reasonable:** *we do not read every line of the libraries we import, so why this?*
...
The clever part is not the clause, it is **the check**. Not "the author confirms they read it" — a box anybody will tick on a Friday. It is *the reviewer picks a line and asks about it*. Ten seconds of Rahul's time, and it converts a promise into a test.
...
**D8 is the sharpest one and it needs the situation spelled out.** Tomas has a failing test. He asks the AI to fix it. There are two ways to make the suite green: change the code so the behaviour is right, or change the test so it agrees with the current behaviour. Both look like a fix in the diff. The second is far easier, so by default it is what happens a distressing proportion of the time.
...
Rahul's version, which is the one people remember:

> **The test is the requirement written in code. You do not get to edit the requirement to pass the exam.**

Ananya adds the check herself, and it is about ordering rather than wording: **read the test diff first, before the code diff.** Once you have read and approved the code, a weakened assertion reads as consistent rather than as suspicious. Look at the tests cold.
...
By six o'clock there are three documents in the repository that did not exist on Friday: a build sequence, a sprint plan, and a definition of done. Tomorrow Tomas starts Step 0.

The handoff is clean:

| Artifact | Goes to | For |
|---|---|---|
| [`implementation-plan-NWD-103.md`](artifacts/implementation-plan-NWD-103.md) | Tomas | One step at a time, with [P18](../../AI-Prompts-Library/phase-4-build/P18-implement-a-story.md) |
...
| `exceptions.sample.json` | Ji-woo | Six rows, four of them awkward |

**And now the honest part, because a chapter about planning that ends with everything going well is not worth reading.**

The plan is good. It is risk-ordered, every step has a command you can type, the riskiest unknown is retired in forty minutes, and the dependency that would have cost Ji-woo half her sprint was caught on day one instead of day seven.
